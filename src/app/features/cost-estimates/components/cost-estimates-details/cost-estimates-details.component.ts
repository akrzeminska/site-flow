...
import { Work } from 'src/app/features/cost-estimates/models/work.model';
...
export class CostEstimatesDetailsComponent implements OnInit {
  ...
  getDataById(id: number) {
    this.costEstimatesService.getById(id).subscribe((costEstimate: any) => {
      if (costEstimate) {
  ...
  getDataAllWorks(id: number) {
    this.costEstimatesService.getAllWorks(id).subscribe((allWorks: any) => {
      if (allWorks) {
